...
packages=("nodejs" "libandroid-support" "openssl" "ca-certificates" "libc++" "c-ares" "libnghttp2" "libicu" "zlib" "brotli")
...
for ARCH in "${ARCHITECTURES[@]}"; do
    ...
    # Build nodejs
    ./build-package.sh -a "$ARCH" -o "$OUTPUT_DIR" nodejs
...
for ARCH in "${ARCHITECTURES[@]}"; do
    ...
    if [ -d "$USR_ROOT" ]; then
        ...
        if [ -f "$USR_ROOT/bin/node" ]; then
            cp "$USR_ROOT/bin/node" "$JNI_OUT_DIR/$JNI_ARCH/libnode.so"
        fi
        ...
        zip --symlinks -r "$JNI_OUT_DIR/$JNI_ARCH/libnode.zip.so" usr/
